window.onload = function() {
  ...
  function onTouchStart(x, y) {
    ...
    if (scene === sceneHome) {
      for (var l = 0; l < levelButtonRects.length-1; l++) {
        if (levelButtonRects[l].contains(x, y)) {
          commandStart(l + 3);
          return;
        }
      }
      if(levelButtonRects[2].contains(x,y)){
        alert("Brought to you by #GamesCon.")
      }
  ...
  function commandHelp() {
    alert("Practice to learn to play the Game. Compete to get higher on Leaderboard");
  }
  ...
  function saveRecords(stackCount, records) {
    ...
    console.log(stackCount,Object.keys(records).length);////SCORE
    if(stackCount==4)
    {
  ...
  function paint() {
    ...
    if (scene === sceneHome) {
      ...
      paintButton('Practice', levelButtonRects[0], 0 === cursorIndex);
      for (var l = 1; l < levelButtonRects.length-1; l++) {
        paintButton(formatLevelText(l), levelButtonRects[l], l === cursorIndex);
      }
      paintButton('About',levelButtonRects[2], 2 === cursorIndex);
